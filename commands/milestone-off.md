...
Disable the per-pane milestone re-injector for the current WezTerm pane.
...
```powershell
python "${CLAUDE_PLUGIN_ROOT}/scripts/milestone-toggle.py" off
```
...
- Exit code non-zero: print stderr and stop. Most likely cause:
  `$WEZTERM_PANE` unset (not in WezTerm).

...
判斷流程：
- 陳述句 + 給明確選擇（"用 A 還是 B？"、"先 X 後 Y 可嗎？"、"方案一/方案二"）
  → `text`，value=具體選擇（如 "做 A"、"先 X"、"選方案二"、"跳過 Y"）。
  Codex 可自主決策，不需把選擇權踢回 Claude。
- 陳述句 + 暗示有下一步可做但無明確分岔（"下一步...", "接下來...", "Week N 用..."）
...
推進語規則：
- **避免猜畫面外細節**：不指定畫面看不到的 file 名 / 數字 / 路徑 / 變數值；
